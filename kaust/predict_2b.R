...
for(i in seq_along(times)) {
  ...
  training <- fread(paste0("~/kaust/Sub-competition_2b/dataset",i,"_training.csv"))
  testing <- fread(paste0("~/kaust/Sub-competition_2b/dataset",i,"_testing.csv"))
  testing$values <- NA_character_
  fit <- readRDS(paste0("kaust/2b_fit-", i, ".RDS"))
  ...
  write.csv(x=testing, file=paste0("kaust/delivery/", teamname, "-2b-", i,".csv"), row.names=FALSE, quote=FALSE)
}

times <- as.data.frame(times)
colnames(times) <- "time in seconds"
write.csv(x=times, file=paste0("kaust/delivery/", teamname, "-2b-time.csv"), row.names=FALSE, quote=FALSE)