...
RFoptions(critical=-1, pivot=PIVOT_NONE, reoptimise=F, optimiser="optim", cores = 1,printlevel = 7)
  ...
  print(system.time(model <- RFfit(RMmatern(nu=NA,scale=NA,var=NA) + RMnugget(var=NA),
                                   x= split_df[1:4],data=split_val[1:4], sub.methods="plain",optim.control=list(trace=6,lmm=3),spC=F)))
  saveRDS(model, file = paste0("~/kaust/models/dataset_update2_",i,".RDS"))
  cat(timestamp())
  cat(rep("\n",10))
sink(NULL)
