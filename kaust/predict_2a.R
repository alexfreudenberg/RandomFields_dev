...
for(i in seq_along(times)) {
  ...
  training <- fread(paste0("~/kaust/Sub-competition_2a/dataset",i,"_training.csv"))
  testing <- fread(paste0("~/kaust/Sub-competition_2a/dataset",i,"_testing.csv"))
  testing$values <- NA_character_
  fit <- readRDS(paste0("kaust/2a-", i, "_fit.RDS"))
  ...
  write.csv(x=testing, file=paste0("kaust/delivery/", teamname, "-2a-", i,".csv"), row.names=FALSE, quote=FALSE)
}

times <- as.data.frame(times)
colnames(times) <- "time in seconds"
write.csv(x=times, file=paste0("kaust/delivery/", teamname, "-2a-time.csv"), row.names=FALSE, quote=FALSE)
