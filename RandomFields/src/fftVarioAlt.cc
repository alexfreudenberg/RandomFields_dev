...
static inline int GetAngleBinX(double angle, double startangle, double SegPerPI, double maxAngle){
  double phi;  
  int kphi;
...
SEXP fftVario3DX(SEXP Coord,
		SEXP Sumvals,
		SEXP Nbvals,
		SEXP VARIABLE_IS_NOT_USED MeanVar,
		SEXP Bin,
		SEXP Nbin,
		SEXP LenT,
		SEXP StepT,
  ...
  for (v1=0; v1<vdim; v1++) {
	...
	for (y = startY, iy=0; y < segmentbase[2];  y+=segmentbase[1], iy++){
	  ...
	  if(!pseudo){
	    ...
	    kphi = GetAngleBinX(phidata, 0, phi[1], PI);
	    phixdata = NEARBY(atan2(d1, -d0) - startphi);
	    kphix = GetAngleBinX(phixdata, 0, phi[1], PI);
	  } else{
	    phidata = NEARBY(atan2(d1, d0) - startphi);
	    kphi = GetAngleBinX(phidata,0, phi[1], TWOPI);
	    phixdata = NEARBY(atan2(d1, -d0) - startphi);
	    kphix = GetAngleBinX(phixdata, 0, phi[1], TWOPI);
	    
	    phiydata = NEARBY(atan2(-d1, d0) - startphi);
	    kphiy = GetAngleBinX(phiydata, 0, phi[1], TWOPI);
	    phixydata = NEARBY(atan2(-d1, -d0) - startphi);
	    kphixy = GetAngleBinX(phixydata, 0, phi[1], TWOPI);
	  }
	  ...
	  for (d2=0.0, z=startZ, iz=0; 
	    ...
	    ktheta = GetAngleBinX(thetadata, starttheta, theta[1], PI);
	    kthetaz = GetAngleBinX(PI - thetadata, starttheta, theta[1], PI);
