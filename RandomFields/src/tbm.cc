...
void rangetbmproc(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
		       simple_range_type *range) {
  
  switch(k) {
    GAUSS_BOXCOX_RANGE;

    // rangetbm_common start, rangetbm_common(cov, range, false);
  case TBM_FULLDIM :
    range->min = 1.0;
    range->max = RF_INF;
    range->pmin = 1.0;
  ...
  case TBM_TBMDIM :
    range->min = RF_NEGINF;
    range->max = RF_INF;
    range->pmin = RF_NEGINF;
    range->pmax = 100;
    range->openmin = false;
    range->openmax = true;
    break;
    
    BooleanRange(TBM_LAYERS);
  ...
  case TBM_LINESIMUFACTOR :
  case TBM_LINESIMUSTEP :
    range->min = 0.0;
    range->max = RF_INF;
    range->pmin = 0.0;
    range->pmax = 10.0;
...
// print("diam %10g %10g %d %d\n", diameter, linesimufactor, *points, GLOBAL.tbm.points);
  ...
  addModel(modelB1, 0, FIRSTDOLLAR);
