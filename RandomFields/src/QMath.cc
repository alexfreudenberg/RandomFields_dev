...
int check_Pow(model *cov){
  ...
  if (sub != NULL && MODELNR(sub) == CONST && !PARAMisNULL(sub, CONST_C)) {
    double p = PARAM0(sub, CONST_C);
    if (p>=0) cov->ptwise_definite = pt_posdef;
  }

  if (cov->ptwise_definite != pt_posdef) {
    sub = cov->kappasub[1];
    if (sub != NULL && MODELNR(sub) == CONST && !PARAMisNULL(sub, CONST_C)) {
      double p = PARAM0(sub, CONST_C);
      int intp = TRUNC(p);
      if (p!=intp || (intp % 2 == 0)) cov->ptwise_definite = pt_posdef;
    }
  }
...
int check_Fmax(model *cov){
  ...
  for (int i=0; i<=1; i++) {
    ...
    if (sub != NULL && MODELNR(sub) == CONST &&!PARAMisNULL(sub, CONST_C)) {
      double p = PARAM0(sub, CONST_C);
      cov->ptwise_definite = p >= 0.0 ? pt_posdef : pt_indef;
      break;
    }
  }
  RETURN_ERR(checkMath(cov));
...
int check_Fmin(model *cov){
  ...
  for (int i=0; i<=1; i++) {
    ...
    if (sub != NULL && MODELNR(sub) == CONST &&!PARAMisNULL(sub, CONST_C)) {
      double p = PARAM0(sub, CONST_C);
      cov->ptwise_definite = p <= 0.0 ? pt_negdef : pt_indef;
      break;
    }
  }
  RETURN_ERR(checkMath(cov));
...
IncludeModel(".fmax", MathDefType, 0, 0, 2, NULL, XONLY,
	 PREVMODEL_I,check_Fmax,rangeMath, PREF_TREND,
	false, SCALAR, PREVMODEL_DEP, falsch, NOT_MONOTONE); 
nickname("fmax");
kappanames("x", REALSXP, "y", REALSXP);
...
IncludeModel(".fmin", MathDefType, 0, 0, 2, NULL, XONLY,
	 PREVMODEL_I,check_Fmin,rangeMath, PREF_TREND,
	false, SCALAR, PREVMODEL_DEP, falsch, NOT_MONOTONE); 
nickname("fmin");
kappanames("x", REALSXP, "y", REALSXP);
setptwise(pt_paramdep); 
...
IncludeModel(".ggamma", MathDefType, 0, 0, 1, NULL, XONLY,
	 PREVMODEL_I,checkMath,rangeMath, PREF_MATHDEF,
	     false, SCALAR, PREVMODEL_DEP, falsch, NOT_MONOTONE); 
nickname("ggamma");
kappanames("x", REALSXP);
...
IncludeModel(".pow", MathDefType, 0, 0, 2, NULL, XONLY,
	 PREVMODEL_I,check_Pow,rangeMath, PREF_MATHDEF,
	false, SCALAR, PREVMODEL_DEP, (ext_bool) false, NOT_MONOTONE); 
nickname("pow");
kappanames("x", REALSXP, "y", REALSXP);
setptwise(pt_paramdep); 
