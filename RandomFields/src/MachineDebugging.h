...
#define CHECKFOR(C)							\
  PRINTF("CHECKFOR: '%s', line %d", __FILE__, __LINE__); checkfor(C)
#define CHECKFOR_BACK(C)						\
  PRINTF("CHECKFORBACK: '%s', line %d", __FILE__, __LINE__);checkfor(C)
*/


// #define ASSERT_GATTER(Cov) assert(TrafoOK(Cov, __FILE__, __LINE__))
#define ASSERT_GATTER(Cov) assert(TrafoOK(Cov, "<change ASSERT_GATTER>", 0))


#define DEBUG(A) {							\
  /* //  printf("at '%s', line %d, %s\n", __FILE__, __LINE__, #A);    */ \
  A;									\
  /*  PRINTF("done (debug): '%s', line %d, %s \n", __FILE__, __LINE__, #A);*/ \
  PRINTF("done (debug): '%s', line %d\n", __FILE__, __LINE__);\
  }


#define COV_DELETE(Cov, save) {						\
    PRINTF("\nCOV_DELETE: '%s', line %d", __FILE__, __LINE__);	\
    COV_DELETE_(Cov, save);}
...
#define ANYEND(C,MESS, SIGN) 						\
  ...
  if (_x==NOERROR){LLPRINT(SIGN, C, " DONE");}				\
    ...
    errorMSG(_x, cov->err_msg, cov->base, msg_0); PRINTF("%s\n", msg_0);} \
  _x;})
