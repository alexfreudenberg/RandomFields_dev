...
int kappaBoxCoxParam(model *cov, int BC) {
  ...
  if (PisNULL(BC)) {						
    ...
    if (cov->nrow[BC]  > 2 && cov->ncol[BC]  > 1)
      SERR("parameters of boxcox must be given as a matrix with 2 rows"); 
    cov->ncol[BC] = total / 2;				
    cov->nrow[BC] = 2;					
    bool notok = false;						
    if (global->gauss.loggauss) {					
...
void kappaGProc(int i, model *cov, int *nr, int *nc){
  ...
  *nr = i == GAUSS_BOXCOX ? 2 : i < DefList[COVNR].kappas ? 1 : -1;
}
...
void rangegaussprocess(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
		       simple_range_type *range) {
  switch(k) {
    GAUSS_BOXCOX_RANGE;
  case GAUSSPROC_STATONLY :
    range->pmin = range->min = 0.0;
    range->pmax = range->max = 1.0; 
    range->openmax = range->openmin = false;
    break;
  default : BUG;  
  }
}
...
#define nsel 4
  ...
  for (i = Nothing - 1; i>=0 && pref[order[i]] > PREF_NONE; i--) {
    ...
    int meth = gaussmethod[unimeth];

    //   printf("METH=%d %d %d\n", meth, CE_INTRINPROC_USER, CE_INTRINPROC_INTERN);
    // assert(meth != CE_INTRINPROC_USER);
  
    addModelKey(cov, meth);
...
int init_binaryprocess( model *cov, gen_storage *s) {
  ...
  if (isnowVariogram(next) || NEXTNR == GAUSSPROC) {
    ...
    for (pi=v=w=0; w<vdimSq; w+=vdimP1, v++, pi = (pi + 1) % npi ) { 
      int idx = v * nmP1;
      if (v < MAXMPPVDIM) cov->mpp.maxheights[v] = 1.0; // maxv
...
int init_chisqprocess(model *cov, gen_storage *s) {
  ...
  if ((err = INIT(sub, 
		  DefList[COVNR].range_multi == rangechisqprocess ? 2 :
		  DefList[COVNR].range_multi == rangetprocess ? 1 : 9999
  ...
  for (int v=0; v<vdim; v++) { 
    ...
    if (cov->mpp.moments >= 0) { 
      ...
      if (cov->mpp.moments >= 1) {
	cov->mpp.mMplus[idx + 1] = 		  
	  DefList[COVNR].range_multi == rangechisqprocess ? m2 
	  : DefList[COVNR].range_multi == rangetprocess ? RF_NAN
	  : RF_NAN;
  ...
  if (DefList[COVNR].range_multi == rangechisqprocess) ReturnOwnField(cov);
  else if (DefList[COVNR].range_multi==rangetprocess) ReturnOtherField(cov,sub);
  else BUG;
...
void rangechisqprocess(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
		       simple_range_type *range) {
  switch(k) {
    GAUSS_BOXCOX_RANGE;
  case CHISQ_DEGREE :
...
void rangetprocess(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
		       simple_range_type *range) {
  switch(k) {
    GAUSS_BOXCOX_RANGE;
  case T_NU : 
