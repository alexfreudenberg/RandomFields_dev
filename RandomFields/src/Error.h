...
#define MERR(X) {LOCAL_ERRLOC_MSG; LPRINT("error msg: ");	\
    errorMSG(X, cov->err_msg, cov->base, ERRMSG); if (PL<PL_ERRORS) {PRINTF("%s\n", ERRMSG);}}
...
#define XERRORNOTDEFINED 21     /* the specification for the covariance &
...
#define XERRORSTATVARIO 23      /*only stat oder variogram allowed as submodel*/
#define XERRORREDUCED 24      /* only stat oder variogram allowed as submodel */
...
#define XERRORNOVARIOGRAM 43     /* variograms aren't allowed in ci & direct */
...
#define XERRORWAVING 48           /* numerical determination of practical range
				    fails */
#define XERRORRESCALING 49        /*attempt to rescale where it isn't possible*/

#define XERRORANISO_T 50         /* anisoT' cannot be given at the same
				time with 'Aniso' oder 'proj' */
#define XERRORANISO 51            /* anisotropic call isn't allowed */
...
#define XERRORDISTANCES 58
#define XERRORBADISO 59
...
#define XERRORNOMULTIVARIATE 102   /* vdim > 1 isn't allowed */
...
#define ERRORDIM 119              /* dim<1 oder dim>MAXDIM */
#define XERRORCEDIM 120              /* dim<1 oder dim>MAXDIM */
...
#define MSGLOCAL_OK 400 /* don't change ordering! (the less the better) */
...
#define ERRC(E) M1ERR(X##E, "default / obvious error : ")
...
#define ERRORBADISO ERRX(ERRORBADISO)
...
#define CERRORBADISO ERRC(ERRORBADISO)
