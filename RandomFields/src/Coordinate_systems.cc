...
coord_sys_enum SearchCoordSystem(model *cov, coord_sys_enum os, 
  ...
  if (n_s == coord_keep) {
    if (!isSameCoordSystem(OWNISO(0), os)) {
      n_s = GetCoordSystem(OWNISO(0)); // no PROTECT( needed
    }
  } else {
...
#define ISOMOD(Z, X, maxangle)				\
  int dim=PREVTOTALXDIM;					\
  X[0]=isomod(Z[0], maxangle);					\
  for (int d = 1; d < dim; d++) X[d] = Z[d]
...
#define EARTH_TRAFO(X, Z, raequ, rpol)			\
  Rcos = (raequ) * COS(Z[EARTH_LATITUDE] * pi180);	\
  X[0] = Rcos * COS(Z[EARTH_LONGITUDE] * pi180);	\
  X[1] = Rcos * SIN(Z[EARTH_LONGITUDE] * pi180);	\
  X[2] = (rpol) * SIN(Z[EARTH_LATITUDE] * pi180)		
...
SEXP GetCoordSystem(SEXP keynr, SEXP oldsystem, SEXP newsystem) {
  ...
  if (knr>=0 && knr <= MODEL_MAX && key[knr] != NULL) {
    ...
    if (os == coord_auto) {
      os = GetCoordSystem(PREVISO(0)); // no PROTECT( needed
    }
    if (n_s == coord_keep) {
