...
void range_hyperplane(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
  ...
  case HYPER_MAR_DISTR :
    range->min = HYPER_UNIFORM;
    range->max = HYPER_BERNOULLI;
    range->pmin = HYPER_UNIFORM;
    range->pmax = HYPER_BERNOULLI;
    range->openmin = false;
    range->openmax = false; 
  break;
  
  case HYPER_MAR_PARAM :
    range->min = 0;
    range->max = RF_INF;
    range->pmin = 0.01;
    range->pmax = 10;
