...
int init_ball(model *cov, gen_storage VARIABLE_IS_NOT_USED *s) {
  ...
  else if (hasRandomFrame(cov) || hasAnyEvaluationFrame(cov)) {
    RETURN_NOERROR;
  }
...
int checktruncsupport(model *cov) {
  ...
  err = CHECK_PASSTF(next, ShapeType, SUBMODEL_DEP, EvaluationType);
  if (err != NOERROR || isTrend(next)) {
  ...
  cov->ptwise_definite = next->ptwise_definite;
  RETURN_NOERROR;
}
...
int init_truncsupport(model *cov, gen_storage *s) {
  ...
  if (hasSmithFrame(cov) || hasAnyPoissonFrame(cov) ||
	...
	err != ERRORILLEGALFRAME) RETURN_ERR(err);
