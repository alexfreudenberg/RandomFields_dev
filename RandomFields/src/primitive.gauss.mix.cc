...
void InverseCauchy(double*x, model *cov, double *v){
  ...
  *v = y <= 0 ? RF_INF : y >= 1.0 ? 0.0 : SQRT(POW(y, -1.0 / gamma) - 1.0);
}
void TaylorCauchy(model *cov) {
  double gamma =  P0(CAUCHY_GAMMA);
  cov->taylor[1][TaylorConst] = cov->tail[0][TaylorPow] = - 2.0 * gamma;
}
...
int initCauchy(model *cov, gen_storage  VARIABLE_IS_NOT_USED *s) {
  ...
  if (hasSmithFrame(cov) || hasPoissonFrame(cov)) {   
    ...
    if (cov->mpp.moments > 0) {
      double sHalf = 0.5 * SurfaceSphere(dim - 1, 1.0),
	mu = 0.5 * dim;
      if (gamma <= mu) SERR("first moment of Cauchy model is infinity.");
      for (int i=1; i<=cov->mpp.moments; i++) {
	//  int_{R^d} 1 / (1+\|x\|^2)^{\gamma * i} =
	// omega_{d-1} int_0^\infty r^{d-1} / (1+r)^{\gamma * i} =
	// omega_{d-1}/2 * B(d/2, i * gamma - d/2)
	double nu = (double) i * gamma;	  
	cov->mpp.mM[i]= cov->mpp.mMplus[i] = sHalf * EXP(logB(mu, nu - mu));
      }
    }
  }
...
void doCauchy(model VARIABLE_IS_NOT_USED *cov, gen_storage VARIABLE_IS_NOT_USED *s) { 
}
...
int checkconstant(model *cov) {
 utilsparam *global_utils = &(cov->base->global_utils);
  ...
  for (int i=0; i<maxv; i++) cov->mpp.maxheights[i] = p[i * vdimP1];
  err = checkkappas(cov);
...
int struct_Gauss(model *cov, model **newmodel) {  
  ...
  switch (cov->frame) {
  case PoissonGaussType : case PoissonType : // optimierte density fuer den Gauss-Fall
    double invscale;
...
int initGauss(model *cov, gen_storage *s) {
  ...
  else if (hasRandomFrame(cov) || hasAnyPoissonFrame(cov) ||
	   hasEvaluationFrame(cov)) { RETURN_NOERROR; }
...
void InversegeneralisedCauchy(double *x, model *cov, double *v) {
  double alpha = P0(GENC_ALPHA), beta=P0(GENC_BETA);
  *v = *x <= 0.0 ? RF_INF : *x >= 1.0 ? 0.0
    : POW(POW(*x, -alpha / beta) - 1.0, 1.0 / alpha);
...
void InverseepsC(double *x, model *cov, double *v){
  double 
    ...
    eps=P0(EPS_EPS);
  *v = *x <= 0.0 ? RF_INF : *x >= 1.0 ? 0.0 :
    POW(POW(*x, -alpha / beta) - eps, 1.0 / alpha);
}
...
void inverse_log_nonstat_stable(double *x, model *cov,
  ...
  double
    y = *x,
    alpha = P0(STABLE_ALPHA),
    z = y >= 1.0 ? 0.0 : y <= 0.0 ? RF_INF : POW( -LOG(y), 1.0 / alpha);
  int d,
    dim = OWNLOGDIM(0);
  for (d=0; d<dim; d++) {
    left[d] = -z;
...
sortsofparam sortof_bistable(model *cov, int k,
 ...
 switch(k) {
  ...
  case BIStablerho :
    return S->rhored_given || origin == mle_conform
      ? IGNOREPARAM : ONLYRETURN;
  case BIStablerhored :
    return S->rhored_given || origin != original_model
      ? ANYPARAM : IGNOREPARAM;
...
void biStablePolynome(double r, double alpha, double a, int dim, double *v ) {
  ...
  if ( dim == 2 || dim == 3 ) {
    *v = alpha*(alpha*(x*(x - 3)  + 1) + 4*(x - 1.0)) + 3;
  }
...
void biStableInterval(double *alpha, double *a, int dim,
  ...
  *left = *right = middle;
  ...
  if ( (scalesratio1 >= 11) || (scalesratio2 >= 11) ) {
    biStableUnderInfLog( 1.0/(POW(2, 1/alpha[1])*a[1] ),
			 alpha, a, dim, &firstmin);
...
getStorage(S ,   bistable); 
 ...
 // beta = betaa + (2 - betaa)*beta_red
  if (S->alphadiag_given) {
   ...
   if (PisNULL(BIStablealpha)) {
     // PMI(cov);
     QERRC2(BIStablealpha, "either '%.50s' or '%.50s' must be set",
	    BIStablealpha, BIStablealphadiag);
    }
 }
   notallowed =
   ...
   if (notallowed) {
     if (s->check) {
       QERRC(BIStablealpha,
	     "This combination of smoothness parameters is not allowed.");
     } else rhomax = 0;
   }
