...
#if __GCC__ >= 2
extern inline void *avltr_insert (avltr_tree *tree, void *item) // not static inline ...
{
  void **p = avltr_probe (tree, item);
  return (*p == item) ? NULL : *p;
}

extern inline void *avltr_replace (avltr_tree *tree, void *item)// not static inline ...
{
  void **p = avltr_probe (tree, item);
  if (*p == item)
    return NULL;
