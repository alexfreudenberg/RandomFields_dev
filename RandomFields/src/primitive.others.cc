...
int checkDivMult(model *cov) {   // R.c
  int err;
  
  if ((err = checkMath(cov)) != NOERROR) RETURN_ERR(err);
  if (cov->ptwise_definite == pt_submodeldep &&
      cov->kappasub[0] != NULL && cov->kappasub[1] != NULL) {
    ptwise_type t = cov->kappasub[1]->ptwise_definite;
    if (t == pt_zero)
      cov->ptwise_definite = MODELNR(cov) == MATHDIV ? pt_mismatch : pt_zero;
    else switch(cov->kappasub[0]->ptwise_definite) {
      case pt_posdef :
	cov->ptwise_definite = t == pt_posdef
	  ? pt_posdef
	  : t == pt_negdef ? pt_negdef : pt_indef;
	break;
      case pt_negdef :
	cov->ptwise_definite = t == pt_posdef
	  ? pt_negdef
	  : t == pt_negdef ? pt_posdef : pt_indef;
	break;
      case pt_zero : cov->ptwise_definite = pt_zero; break;
      default : cov->ptwise_definite = pt_indef;
      }
  }
...
Types Typebind(Types required, model *cov,
		  isotropy_type VARIABLE_IS_NOT_USED iso) {
  //  printf("c:%s %d %d %d %d \n", TYPE_NAMES[required], !isNegDef(required),
  //	 !PisNULL(1), cov->kappasub[0] != NULL, cov->kappasub[1] != NULL);
  //  printf("%d %lu %lu", !PisNULL(1), cov->kappasub[0], cov->kappasub[1]);
  //  PMI(cov);

  if (isTrend(required)) return required;
  if (!isNegDef(required) ||
      !PisNULL(1) || cov->kappasub[0] != NULL || cov->kappasub[1] != NULL
      ) return BadType;
  //  printf("ok\n");
...
int check_c(model *cov){
  ...
  if (PisNULL(CONST_C)) SERR("value of constant not set.");
  if (negdef) {
...
void kappa_declare(int i, model *cov, int *nr, int *nc){
  *nr = i >= DefList[COVNR].kappas? -1 : SIZE_NOT_DETERMINED;
