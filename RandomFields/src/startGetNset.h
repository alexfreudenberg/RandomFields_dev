...
int IncludeModelR(const char *name, Types type, int minsub, int maxsub,
		 int kappas, size_fct kappasize,
		 domain_type domain, isotropy_type isotropy, checkfct check,
		 rangefct_multi range_multi, // difference to previous
		 pref_type pref,  int internal,
...
Types TypeDefault(Types required, model *cov, isotropy_type required_iso);
bool setUnreducedOrDist(model *cov);
Types TypeGaussMethod(Types required, model *cov, isotropy_type required_iso);
Types TypeNormedProcess(Types required, model *cov, isotropy_type required_iso);
