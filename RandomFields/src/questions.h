...
bool isPosDef(model *cov);
bool isVariogram(model *cov);
...
bool equalsManifold(Types type);
...
bool isDollarProc(model *cov);
bool isAnyDollar(model *cov);
#define isPlus(cov) (MODELNR(cov) == PLUS)
#define isMal(cov) (MODELNR(cov) == MULT)
#define isConst(cov) (MODELNR(cov) == CONST)
...
bool isCallingSet(model *cov);
bool TrafoOK(model *cov, const char *file, int line);
bool TrafoOK(model *cov);
