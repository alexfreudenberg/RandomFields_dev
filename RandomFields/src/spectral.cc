...
void range_spectral(model VARIABLE_IS_NOT_USED *cov, int k, int i, int j,
...
int init_spectral(model *cov, gen_storage *S){
  ...
  if (LocDist(cov)) RETURN_ERR(ERRORFAILED);
  if (dim > MAXTBMSPDIM) {
...
void do_spectral(model *cov, gen_storage *S) {  // in two dimensions only!
  ...
  double inct, VV,
    ...
    *res = cov->rf;
  Long nt, nx, total = Loctotalpoints(cov),
    spatialpoints = Locspatialpoints(cov);
  SAVE_GAUSS_TRAFO;
...
//  print("%d %10g %10g\n", s->grid, s->phistep2d, s->phi2d); assert(false);
  ...
  if (grid) {
    switch (origdim) {
    case 4 : 
      gridlent = (int) gr[3][XLENGTH];
      FALLTHROUGH_OK; 		
    case 3 : 
      gridlenz = (int) gr[2][XLENGTH];
      FALLTHROUGH_OK; 		
    case 2 : 
      gridleny = (int) gr[1][XLENGTH];
      FALLTHROUGH_OK; 		
    case 1 : 
      gridlenx = (int) gr[0][XLENGTH];
      break;
    default: BUG;
    }
  }
