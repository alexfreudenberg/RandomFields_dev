...
static inline int GetAngleBin(double angle, double startangle, double SegPerPI, double maxAngle){
  double phi;  
  int kphi;
 ...
 */
  ...
  int
    ...
    lenT = INTEGER(LenT)[0], 
    back_totalbins = segmentEmpVario * vdim * vdim,
    nstepTstepT = stepT * nstepT,
    timecomp = nstepTstepT==0 ? 1 : 2,
    halfnbin = nbin / 2;
  bool pseudo = LOGICAL(Pseudo)[0],
    simple = !pseudo && timecomp==1;
 

  SEXP Back = PROTECT(allocMatrix(REALSXP, back_totalbins, 2 + pseudo));
  
  double
    *back = REAL(Back),
    *emp_vario = back + back_totalbins * EV_FFT_EV,
    *n = back + back_totalbins * EV_FFT_N,
    *mV = back + back_totalbins * EV_FFT_VAR;

  int  maxi[3],
    Nphi = phi[1]==0 ?  1 : (1 + !simple) * ((int) phi[1]),
    NphiNbin= Nphi * nbin,
    Nphihalf = simple || phi[1]==0 ? 0 : (int) phi[1],
 ...
 double *xx[3],step[3], maxbinsquare, 
    startphi = phi[0] - PI / (double) ((1 + simple)*Nphi), 
    starttheta = 0.0,
  ...
  //print("2,..");
   ...
   Long rep = 0;
   for (int i=0; i<3; i++, rep+=3) xx[i] = coord + rep;
  ...
  for (int i=0; i<totalbinsvdimSq; i++) emp_vario[i] = n[i]=0.0;
  if (pseudo) {
    meanVar = REAL(MeanVar);
    for (int i=0; i<totalbinsvdimSq; i++) mV[i]=0.0;
    assert(mV[0] == 0.0);
  }
  //  for (int i=0; i<80; i++) printf("%f ", emp_vario[i]);
  //  printf("\ntot=%d\n", totalbinsvdimSq); // BUG; ERR("hhh");
  ...
  //  printf("sEV %d %d\n", segmentEmpVario, segmentbase[7]);

  for (int v1=0; v1<vdim; v1++) {
    for (int v2=0; v2<vdim; v2++,
	   sumvals += segmentbase[7], 
	   nbvals += segmentbase[7],
	   meanVar += segmentbase[7],
	   emp_vario += segmentEmpVario,
	   n += segmentEmpVario,
	   mV += segmentEmpVario
	 ) {
      //    printf("\nv1=%d v2=%d %d %d\n", v1, v2, segmentbase[7], segmentEmpVario);
      //    for (int i=0; i<80; i++) printf("%i:%f ", i, emp_vario[i]);
      //       printf("\nn:\n");
      //      for (int i=0; i<80; i++) printf("%i:%f ", i, n[i]);
      //   printf("\nsEV %d %d\n", segmentEmpVario, segmentbase[7]);
	...
	binidx = 0,
	x=startX;
      for (int ix=0; x < segmentbase[1]; x++, ix++) {
	//	printf("ix=%d\n", ix);
	double d0 = ix * step[0],
	  psq0 = d0 * d0;   
	if (psq0 > maxbinsquare) continue;      
	int startY = x,
	  y = startY;
	for (int iy=0; y < segmentbase[2];  y+=segmentbase[1], iy++){
	  double 
	    d1 = iy * step[1],
	    psq1 = d1 * d1 + psq0;
	  if (psq1>maxbinsquare) continue; 
	  int startZ = y;
	  // angles
	  int kphi, kphix, phiydata, phixydata,
	  ...
	  double d2=0.0;
	  int z=startZ;
	  for (int iz=0; z < segmentbase[3];
	       z+=segmentbase[2], iz++, d2 += step[2]){
	    int cur,
	      startT = z;
	    double psq2 = d2 * d2 + psq1;
	    if (psq2 > maxbinsquare) continue;
	    ...
	    for (int deltaT=0, kT=0; deltaT<=nstepTstepT; 
		 deltaT += stepT, kT += nbinNphiNtheta) {
	      //print("kT %d, stepT %d, nsteptT %d, deltaT %d", kT, *stepT, *nstepT, deltaT); 	 
	      //print("kT %d --", kT);
	      //timecomp = x==0 ? 1 : 2;
	      
	      int startrep = startT + deltaT * segmentbase[3];
	      for (int k=1; k <= timecomp; k++, startrep += segmentbase[5]){ 
...
#define ADDPSEUDO				\
  emp_vario[Idx] += sumvals[jj];		\
  mV[Idx] += meanVar[jj];			\
  n[Idx] += nbvals[jj]

		  //if (emp_vario[Idx] != mV[Idx] || emp_vario[0] != mV[0]) {printf("Idx=%d jj=%d totbin=%d %d ev=%f mV=%f, val=%f meanVar=%f %d\n", Idx, jj,totalbins, back_totalbins, emp_vario[Idx], mV[jj], sumvals[jj], meanVar[jj], totalbinsvdimSq); BUG;} else printf(":");
		  ...
		  if (pseudo) {
		    ...
		    if(iy > 0) {
		      int
			kxynbin = NA_INTEGER, // to avoid compiler message
		    ...
		    //		    printf("%d %d %d ", base, Idx, jj);

		    //printf("rep =%d [%d %d; %d]  v1=%d %d [%d] x=%d %d %d deltaT=%d %d\n", 
		    //rep, startrep, segmentbase[7], segmentbase[6],
		    //v1, v2, vdim, x, y, z, deltaT, k);
		    
		    //		    printf("base=%d %d %d %f %f %f %f\n", base, Idx, jj, sumvals[jj], nbvals[jj], emp_vario[base + Idx], n[base + Idx]);
		    emp_vario[base + Idx] += sumvals[jj];
		    n[base + Idx] += nbvals[jj];  //	
		    //		    printf("base=%d %d %d %f %f %f %f\n", base, Idx, jj, sumvals[jj], nbvals[jj], emp_vario[base + Idx], n[base + Idx]);
		    jj += segmentbase[4];
		    if (ix > 0 && (iy > 0 || iz > 0)) {
		      basX = binidx + (kphix + Nphihalf * kM1) * nbin;
		      emp_vario[basX + Idx] += sumvals[jj];
