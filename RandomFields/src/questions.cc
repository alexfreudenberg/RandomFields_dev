...
bool isPrevModelD(domain_type dom) {
  return dom == PREVMODEL_D || dom == PREV_SUB_D ;
}
bool isPrevModelD(defn *C){
  assert(LASTSYSTEM(C->systems[0]) >= 0);
  bool is = isPrevModelD(DOM(C->systems[0], 0));
  assert(!is || LASTSYSTEM(C->systems[0]) == 0);
 return is;
}

// bool equalsSubModelD(domain_type dom) { return dom == SUBMODEL_D;}
bool isSubModelD(domain_type dom) {
  return dom == SUBMODEL_D || dom == PREV_SUB_D ;
}
bool isSubModelD(defn *C){
  assert(LASTSYSTEM(C->systems[0]) >= 0);
  bool is = isSubModelD(DOM(C->systems[0], 0));
  assert(!is || LASTSYSTEM(C->systems[0]) == 0);
  return is;
}
...
bool isPrevModelI(isotropy_type iso) {
  return iso == PREVMODEL_I || iso == PREV_SUB_I ;
}
bool isPrevModelI(defn *C){
  assert(LASTSYSTEM(C->systems[0]) >= 0);
  bool is = isPrevModelI(ISO(C->systems[0], 0));
  assert(!is || LASTSYSTEM(C->systems[0]) == 0);
 return is;
}

bool equalsSubModelI(isotropy_type iso) { return iso == SUBMODEL_I;}
bool isSubModelI(isotropy_type iso) {
  return iso == SUBMODEL_I || iso == PREV_SUB_I;
}
bool isSubModelI(defn *C){
  assert(LASTSYSTEM(C->systems[0]) >= 0);
  bool is = isSubModelI(ISO(C->systems[0], 0));
  assert(!is || LASTSYSTEM(C->systems[0]) == 0);
  return is;
}
...
bool isDefCL(typefct isTypus, model *cov, bool OneSystem) {
  ...
  //  printf("onesysetm %d %d %d\n", OneSystem, n, C->TypeFct != NULL);
  
  if (OneSystem && n != 1) return false;
  Types type = SYSTYPE(sys, 0);
  if (equalsManifold(type)) return false;
  //  if (C->TypeFct != NULL) {
  //    //   printf("call of TypeFct in isDefCL by is(%s)\n", NAME(cov));
  //    return false;  }
  //  printf("name = %s %d\n", NAME(cov), isTypus(type));
  if (!isTypus(type)) return false;
...
QuestionIsNow(PosDef, PosDefType || isTcf(type), false)
QuestionIsNow(Variogram, VariogramType || isPosDef(type), false)
...
bool isnowTrendParam(model *cov, int i) {
  ...
  return
    isnowTrend(cov) && (SortOf(cov, i, 0, 0, original_model) == TRENDPARAM);
}
...
int maxdim_notok(model *cov) {
  ...
  for (int s=0; s<=last; s++) {
    int max = OWNMAXDIM(s);
    if (max >= 0 && max < OWNLOGDIM(s)) return s;
  }
  return -1;
}
...
bool hasVarOnly(model *cov) {
  // gleichzeitiges Geben von einer Funktion fuer var und einer fuer scale
  // kann verstanden werden als beziehend fuer die jeweiligen Koordinaten
  if (cov == NULL || !isDollar(cov)) BUG;
  if (!PisNULL(DSCALE) && P0(DSCALE) != 1.0) return false;
  if (!PisNULL(DANISO) || !PisNULL(DPROJ) ||
      cov->kappasub[DANISO] != NULL || cov->kappasub[DPROJ] != NULL)
    return false;
  return true;
}
...
bool isDollar(model *cov) {
  assert(cov != NULL);
  int nr=COVNR;
  return nr >= FIRSTDOLLAR && nr <= LASTDOLLAR;
}
...
bool isAnyDollar(model *cov) {
  assert(cov != NULL);
  int nr=COVNR;
  return (nr >= FIRSTDOLLAR && nr <= LASTDOLLAR) || nr == DOLLAR_PROC;
}
...
Types TypeConsistency(Types requiredtype, model *cov, isotropy_type requirediso) {
  ...
  assert(!isSubModelI(requirediso));
  ...
  //   PMI(cov); 
    //
#ifdef TypeDebug
    PRINTF("MANIFOLD %s requ:%s, %s (own=%s)\n", NAME(cov), TYPE_NAMES[requiredtype], ISO_NAMES[requirediso], ISO_NAMES[OWNISO(0)]);
#endif
...
bool EmptySubOK(int covnr) {
  return covnr == RANDOMCOIN_USER || covnr == AVERAGE_USER ||
    covnr == POISSONPROC || covnr == SMITHPROC;
}


bool isCallingSet(model *cov) { // is calling model correctly set in 
  // subsequent models??
  ...
  for (int i=0; i<maxsub; i++) {    
	...
	return false;
      ...
      j++;
      if (sub->calling != cov) {
	PRINTF("%d%s submodel->calling != cov\n", i+1, TH(i+1));
	PMI0(cov);//
	return false;
      }
      if (!isCallingSet(sub)) {  
  ...
  for (int i=maxsub; i < MAXSUB; i++) {
    if (cov->sub[i] != NULL) {
      WARN4("%.50s: %d%.50s submodel not NULL although nsub=%d",
	    NAME(cov), i+1, TH(i+1), nsub);
      BUG;
    }
  }
  ...
  if (cov->Splus != NULL && cov->Splus->keys_given) {
	...
	if (sub->calling != cov) {
	  PRINTF("sub->calling != cov\n");
	  PMI0(cov); //
	  return false;
	}
	if (!isCallingSet(sub)) return false;
      }
    }
  }
  return true;
...
bool TrafoOK(model *cov) {// check other things, too, besides gatter ?
  ...
  return ((FIRSTGATTER <= gatter && gatter <= LASTGATTER)
	  &&
	  ((trafo >= FIRST_TRAFO && trafo <= LAST_TRAFO) || trafo == UNSET))
    && cov->checked;
}
...
bool TrafoOK(model *cov, const char *file, int line) {// check other things, too, besides gatter ?
  ...
  if (!false && !ok) {
    PMI0(cov->calling); //
    PMI0(cov);//
    PRINTF("%s: not: %d < %d <= %d UND (%d <= %d <= %d oder %d == %d) UND checked=%d in %s, line %d\n", 
...
bool isIntrinsicProc(model *cov) {
  int nr = COVNR;
  return nr == CE_INTRINPROC_INTERN || nr == CE_INTRINPROC_USER;
}

bool isNotEstimable(int pt) {
  return pt == ONLYRETURN || pt == DONOTVERIFYPARAM  || pt == IGNOREPARAM ||
    pt == FORBIDDENPARAM;
}
