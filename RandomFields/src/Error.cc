...
void errorMSG(int err, errorstring_type errorstring, KEY_type *KT,
  ...
  case XERRORTYPECONSISTENCY :
     SCPY(m,"incorrect choice of submodel (type or dimension inconsistency)"); break;
  ...
  case XERRORRESCALING:
    SCPY(m,"practical range undefined");
    break;
  case XERRORANISO_T :
    SPRINTF(m, "'%.50s' may not be given at the same time with '%.50s' or '%.50s'", 
	    DefList[FIRSTDOLLAR].kappanames[DANISO], 
	    DefList[FIRSTDOLLAR].kappanames[DAUSER], 
	    DefList[FIRSTDOLLAR].kappanames[DPROJ]);
    break;
  case XERRORANISO:
 ...
 case XERRORANISO_DET:
  ...
  case XERRORNONSTATSCALE:
    ...
    break;
   case XERRORDISTANCES:
    SCPY(m,"distances instead of coordinates may not be given in this context");
    break;
 ...
 case XMSGLOCAL_NOPOSITIVEROOT :
  ...
  case TOOLS_UNKNOWN_CHAR:
    SCPY(m,"unknown type of second order characteristic");
     break;
  case TOOLS_METHOD:
    SPRINTF(m,"unknown method in empirical variogram calculation.%.90s",
	    CONTACT);
    break;
...
void OnErrorStop(int Err, model *cov) {
  ...
  errorMSG(Err,
	   ...
	   ? cov->err_msg
	   : cov->base->error_causing_cov->err_msg,
