...
void includeOtherModels() {  
 ...
 // *******************
  ...
  SELECTNR =  // to do: replace by parameter in '+', selecting the 'type' or
    // 'models'
    IncludeModel("select", TcfType, 1, MAXSUB, 1, NULL, PREV_SUB_D, PREV_SUB_I,
 ...
 ANGLE = 
  ...
  change_sortof(COVARIATE_RAW, IGNOREPARAM);
  ...
  RANDOMSIGN = 
    IncludeModel("sign",  ShapeType, 1, 1, 1, NULL, XONLY, PREVMODEL_I,
		 ...
		 false, SCALAR, SUBMODEL_DEP, (ext_bool) SUBMODEL_DEP,
		 NOT_MONOTONE);
  //nickname("");
...
#define balltest !true  
 ...
 //     sondern monotone im eindimensionalen
  ...
  // ALWAYS FIRST WITHIN FUNCTION WITHOUT GENUINE I NIT


  COVFCTN = // ALWAYS FIRST WITHIN FUNCTION WITHOUT GENUINE I NIT
    IncludeModel("Covariance", InterfaceType, 1, 1, 0, NULL, XONLY, PARAMDEP_I,
  ...
  COVMATRIX = 
    IncludeModel("CovMatrix", InterfaceType, 1, 1, 0, NULL, XONLY, PARAMDEP_I,
  ...
  */
  ...
  DOLLAR_PROC  // also ok for distances??
    = IncludeModel("$proc", ProcessType,		   
		   ...
		   MON_SUB_DEP);
  // do not change Order!!
  addSpecific(FIRSTDOLLAR);
  RandomShape(2, structSproc, initSproc, doSproc, true, true, true);
  AddVariant(GaussMethodType, UNREDUCED);

  
  PLUS_PROC =  // also ok for distances??
    IncludeModel("plusproc", ProcessType, 1, MAXSUB, 1, NULL, 
		  XONLY, UNREDUCED,
		 checkplusproc, rangeplus, PREF_ALL, 
		 true, SUBMODEL_DEP, SUBMODEL_DEP, (ext_bool) SUBMODEL_DEP,
		 MON_SUB_DEP);
  addSpecific(PLUS);
  ...
  // 
  PROD_PROC = // also ok for distances??
  ...
  MULT_PROC =  // also ok for distances??
  ...
  M_PROC = // also ok for distances??
    IncludeModelR("matrixproc", ProcessType, 1, MAXSUB, 2, kappaM,
		  XONLY, UNREDUCED,
  ...
  AVERAGE_USER = 
    IncludeModelR(METHOD_NAMES[Average], GaussMethodType, 1, 2, 3, kappaGProc, 
		 XONLY, UNREDUCED,
  ...
  CIRCEMBED = // und die anderen fehlen auch noch !!
    IncludeModelR(METHOD_NAMES[CircEmbed], GaussMethodType, 1, 1, 13, kappa_ce,
		 XONLY, UNREDUCED,
  ...
  CE_CUTOFFPROC_USER =  
    IncludeModelR(METHOD_NAMES[CircEmbedCutoff], GaussMethodType, 1, 1, 15, 
  ...
  CE_INTRINPROC_USER =  
    IncludeModelR(METHOD_NAMES[CircEmbedIntrinsic], GaussMethodType, 
  ...
  DIRECT = 
    IncludeModelR(METHOD_NAMES[Direct], GaussMethodType, 
		 1, 1, 1, kappaGProc, XONLY, PARAMDEP_I,
  ...
  setDI(NULL, allowedIdist, setUnreducedOrDist);
  addTypeFct(TypeGaussMethod);

  
  HYPERPLANE_USER  = 
    IncludeModelR(METHOD_NAMES[Hyperplane], GaussMethodType, 1, 1, 6, kappaGProc,
		 XONLY, UNREDUCED,
  ...
  NUGGET_USER  = 
    IncludeModelR(METHOD_NAMES[Nugget], GaussMethodType, 
		 1, 1, 3, kappaGProc, XONLY, PARAMDEP_I,
  ...
  SEQUENTIAL = 
    IncludeModelR(METHOD_NAMES[Sequential], GaussMethodType, 1, 1, 3,
		 kappaGProc, 
		 XONLY, UNREDUCED,
  ...
  SPECTRAL_PROC_USER = 
    IncludeModelR(METHOD_NAMES[SpectralTBM], GaussMethodType,  1, 1, 5,
		 kappaGProc,
		 XONLY, UNREDUCED,
  ...
  SPECIFIC = 
    IncludeModelR(METHOD_NAMES[Specific], GaussMethodType, 1, 1, 1, kappaGProc,
		 XONLY, UNREDUCED,
  ...
  TBM_PROC_USER = 
    IncludeModelR(METHOD_NAMES[TBM], GaussMethodType,  1, 1, 9, tbm_kappasproc, 
		 XONLY, UNREDUCED, 
  ...
  BROWNRESNICKPROC =
    IncludeModel(BR_NAME, BrMethodType, // ProcessType
		 1, 2, 3, NULL, 
		 XONLY, UNREDUCED, 
		 checkBrownResnickProc, range_mpp, PREF_NOTHING,  
		 false, SCALAR, MAXMPPDIM, (ext_bool) false, MON_MISMATCH);
  ...
  BINARYPROC = // direct an Gauss gekoppelt!!
    IncludeModelR("binaryprocess", NormedProcessType, 1, 1, 3,
		 kappa_binaryprocess, XONLY, PARAMDEP_I,
  ...
  POISSONPROC =
    IncludeModel("poisson", PoissonType, 1, 2, 2, NULL,
		 XONLY, UNREDUCED,  
		 check_poisson, range_poisson, PREF_NOTHING,
		 false, SUBMODEL_DEP, MAXMPPDIM, (ext_bool) false,
		 MON_MISMATCH);
  subnames("shape", "phi");
  kappanames("intensity", REALSXP, "support", REALSXP);
  ...
  SMITHPROC =
    IncludeModel(SMITH_NAME, SmithType, 1, 2, 3, NULL, XONLY, UNREDUCED,
  ...
  CHI2PROC =
    IncludeModelR("chi2", ProcessType, 1, 1, 2, kappaGProc, XONLY, UNREDUCED,
  ...
  TPROC =
    IncludeModelR("t", ProcessType, 1, 1, 2, kappaGProc, XONLY, UNREDUCED,
