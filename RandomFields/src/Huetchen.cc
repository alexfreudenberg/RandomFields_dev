...
void Zhou(double *x, int *info, model *cov, double *v) {// pointshapetype
 ...
 // PMI(cov);
  ...
  ///  if (*v > (1  + 1e-12) * pts->mpp.unnormedmass) printf("violating v=%10e %10e\n", *v,  pts->mpp.unnormedmass);
...
int check_Zhou(model *cov) {
 ...
 model *shape = cov->sub[PGS_FCT],
  ...
  // isotropy is the property -- user shouldn't define unisotropic functions!
...
getStorage(pgs ,   pgs); 
  ...
  if (!isProcess(shape)) // war: if (cov->randomkappa) 
    SERR1("shapes must be a (stationary) process in stationary modelling.%.90s",
	  CONTACT);
...
int check_standard_shape(model *cov) {
  ...
  if (shape->randomkappa &&
      (MODELNR(shape)!= TRUNCSUPPORT || cov->kappasub[TRUNC_RADIUS] != NULL))
    SERR1("random support for '%.50s' not allowed yet. Set the argument 'support' case of 'RPpoisson'.", NICK(cov));
...
int struct_standard_shape(model *cov, model **newmodel){
  ...
  if (!hasAnyPoissonFrame(shape) && !hasSmithFrame(shape)) ILLEGAL_FRAME;
...
int init_standard_shape(model *cov, gen_storage *S) {  
  ...
  //  printf("init standard shape %d %d\n", cov->mpp.moments, hasSmithFrame(cov));
  ...
  if (hasAnyPoissonFrame(cov)) {
    ...
    if (pgs->estimated_zhou_c)
      SERR1("random shapes in standard approach not coded yet%.90s", CONTACT);
  } else BUG;
...
int check_Ballani(model *cov){
  // printf("checkballani\n");
  //
  RETURN_ERR(ERRORNOTPROGRAMMEDYET); // the paper to write
