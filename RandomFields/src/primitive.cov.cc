...
/* see Yaglom ! */
  // nu==0.0 ? 1.0 : // not allowed anymore;
	// other wise densityBessel (to define) won't work
  if (nu >= 0.0) {
  ...
  case PoissonGaussType :
    {
      ...
      int owndim = OWNLOGDIM(0);
      addModelX(newmodel, FIRSTDOLLAR);
      if (owndim < dim) {
	addModelKappa(*newmodel, DSCALE, SCALESPHERICAL);
	model *scale = (*newmodel)->kappasub[DSCALE];
	kdefault(scale, SPHERIC_SPACEDIM, (double) owndim);
	kdefault(scale, SPHERIC_BALLDIM, (double) dim);
      } else {
	if (owndim == dim) {
	  kdefault(*newmodel, DSCALE, 0.5);
	} else BUG; 
      }
    }
    break;
...
void Inversedewijsian(double *x, model *cov, double *v){ 
  double alpha = P0(DEW_ALPHA);
  *v = *x < 0.0 ?  0.0 : POW(EXP(*x) - 1.0, 1.0 / alpha);    
} 
...
int initlsfbm(model *cov, gen_storage VARIABLE_IS_NOT_USED *s) {   
  ...
  if (PisNULL(LOCALLY_BROWN_C)) {
    ...
    if (PL >= PL_DETAILSUSER) {
      PRINTF("'%s' in '%s' equals %10g for '%s'=%10g\n", KNAME(LOCALLY_BROWN_C),
...
void Inverselsfbm(double *x, model *cov, double *v) {
  if (*x > 1.0)
    ERR1("the coordinate distance in '%.50s' must be at most 1.", NICK(cov));
  double alpha = P0(LOCALLY_BROWN_ALPHA);
  *v = *x > QVALUE ? 0.0 : POW(QVALUE - *x, 1.0 / alpha);
}
...
void InversegenBrownian(double *x, model *cov, double *v) {
  double 
    ...
    beta =  P0(BROWN_GEN_BETA),
    delta = beta / alpha;
  *v = *x <= 0.0 ? 0.0 : POW(POW(*x + 1.0, 1.0/delta) - 1.0, 1.0/alpha); 
}
...
getStorage(S ,   biwm); 
  ...
  if  (S->cdiag_given) { // cdiag or rho given
    ...
    if (!ISNAN(c[i11]) && !ISNAN(c[i22]) && (c[i11] < 0.0 || c[i22] < 0.0))
      QERRC2(GNEITING_C,
	     "variance parameter %.50s[0], %.50s[2] must be non-negative",
...
sortsofparam sortof_biGneiting(model *cov, int k, int VARIABLE_IS_NOT_USED row,
  ...
  case GNEITING_CDIAG :
    ...
    //    printf("%d\n",  S->cdiag_given);
    //printf("%d\n", origin != original);
    return S->cdiag_given || origin != original_model ? VARPARAM : IGNOREPARAM;
  case GNEITING_RHORED :
    return S->cdiag_given || origin != original_model ? ANYPARAM : IGNOREPARAM;
  case GNEITING_C:
...
void Inverselgd1(double *x, model *cov, double *v) {
  double
    ...
    alpha = P0(LGD_ALPHA),
    beta=P0(LGD_BETA);
  if (y <= 0.0) {*v = RF_INF; return;}
  if (y >= 1.0) {*v = 0.0; return;}
...
int structpower(model *cov, model **newmodel) {
  if (OWNLOGDIM(0) == 1 && P0(POW_ALPHA) == 1)
    return structCircSph(cov, newmodel, 1);
  else return ERRORFAILED;
}

int initpower(model *cov, gen_storage VARIABLE_IS_NOT_USED *s) {
  if (hasSmithFrame(cov) || hasPoissonFrame(cov)) {
    double alpha = P0(POW_ALPHA),
      y = alpha + 1;
    for (int i=0; i<=cov->mpp.moments; i++) {
      double x = (double) i + 1.0;
      cov->mpp.mM[i] = cov->mpp.mMplus[i] = EXP(logB(x, y));
    }
    cov->mpp.moments = INFTY;
    RETURN_NOERROR;
  }
