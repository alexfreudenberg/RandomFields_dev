...
#Print("detrendedfluc")
  ...
  if (any(mode=="plot" | mode=="interactive"))
  {
    ...
    screens <- split.screen(erase=FALSE,
                            figs=cbind(screens[-plots-1], screens[-1], 0, 1))
...
RFfractaldim <- function(x, y = NULL, z = NULL, data, grid, 
  ...
  if (do.vario) {
    ...
    if (bin.not.given <- length(bins) == 0) {
      if (ct$grid) {
        step <- min(ct$x[2, ])
        end <- ct$x[2, ] * ct$x[3, ] / 4
 ...
 #   Print(bins, step, end, edge.lengths)
    ...
    ev <- rfempirical(x=x, y=y, z=z, T=T, data=data, grid=ct$grid, bins=bins,
    ...
    if (length(idx) < vario.n) {
      ...
      if (bin.not.given) 
        note("A good choice for 'bins' was not found. Please try out the arguments manually. Current value is 'bins = ", paste(bins, collapse=", "), "' so that only the bins with center value ", paste(l.binvario[idx], collapse=", "), " possess values of the variogram cloud.")
      else
        note("Current value of 'bins' leads to ", length(idx), " values for the empirical variogram. Consider trying out different arguments.")
    }
