...
default.image.par <- function(data.range, var.range, legend=TRUE) {
  ...
  if (is(var.col, "try-error")) {
    ...
    if (is(var.col, "try-error")) {
      ...
      data.colour <- RColorBrewer::brewer.pal(9, "Reds")
    }    
  } else {
...
prepareplotRFsp <- function(x, vdim, select, plot.variance,
  ...
  if (graphics$split_screen) {  
    ...
    scr.main <- matrix(nrow = figs[2],
                       split.screen(erase=FALSE, split.main, scr[1]),
  ...
  if (plot.legend) {
    ...
    for (i in c("data", if (plot.variance) "var")) {
      if (graphics$split_screen) {  
        ...
        scr.leg <- split.screen(erase=FALSE, figs=c(1, len.sel))
        scr.legends <- c(scr.legends, scr.leg)
        for (jx in 1:length(scr.leg)) {
          screen(scr.leg[jx], new=FALSE)
  ...
  for (m in m.range) {
    ...
    for (jx in 1:length(select.variables)) {       
      ...
      for (ix in 1:nrow(all.i)) { 
        ...
        if (graphics$split_screen) {
          screen(image.par$scr.main[ix, jx], new=FALSE)
        ...
        if (image.par$legend && ix == 1 && legends$do[jx]) {
          ...
          if (graphics$split_screen) {
            screen(legends[[dv]]$scr[jx], new=FALSE)
        ...
        if (image.par$legend && ix == 1) {
          if (do.plot.arrows) {
            ...
            if (graphics$split_screen) {
              ...
              screen(image.par$scr.legends[jx], new=FALSE)
...
RFplotSimulation1D <-  function(x, y, nmax=6,
  ...
  if (graphics$split_screen) scr <- split.screen(erase=FALSE, c(n,1))
  else {
    scr <- NULL
  ...
  for (i in 1:n){
    if (graphics$split_screen) screen(i, new=FALSE)
    if (make.small.mar) {
