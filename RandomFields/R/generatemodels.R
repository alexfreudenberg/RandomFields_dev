...
##       x[i] <- paste(x[i], "CheckProj(proj, subst)", sep="")
      ...
      x[i] <- paste(x[i], "CheckChar(", names[i], ", subst, ",
                    ...
                    havedistr, ")", sep="")
   else if (ismath && names[i] != "name")
       x[i] <- paste(x[i], "CheckMaths(", names[i], ", subst, ",
                    havedistr, ")", sep="")    
   else if (ismath && names[i] == "name")
     x[i] <- paste(x[i], "CheckChar(", names[i], ", subst=subst, distr=",
                    havedistr, ")", sep="")
...
E <- function(n) if (n >= 0) n else NEGATIVE_NAMES[-n] # nicht + 1
...
rfGenerateModels <- function(package="RandomFields", assigning,
  ...
  for (i in 1:length(NEGATIVE_NAMES)) 
    if (i != -get(NEGATIVE_NAMES[i]))  # nicht + 1
       stop("NEGATIVE_NAMES do not match values")
  ...
  while (i <= va) {
    ...
    if (A$internal[i]) {
  #    Print("internal", nick)
      internal.nick <- paste("i", internal.nick, sep="")
    } #else Print(nick)
    ...
    # to the function:
     ...
     text.assign.class <-
      paste(internal.nick, " <- new(CLASS_RM, \n\t",
         ...
         "maxdim = ",if(A$maxdim[i]>diminf) Inf else E(A$maxdim[i]),",","\n\t",
         "vdim = ",      E(A$vdim[i]),                                  "\n\t",
         ")",
         sep="")
...
kind <- function(Zeilen, i, start, cont="", ignore=" ",
  ...
  if (substr(Zeilen[i], 1, nchar(start)) == start) {
    ...
    j <- 2
    if (j > nchar(s)) return(NULL)
    
    while (j <= nchar(s) && substr(s, j, j) != endofname) j <- j + 1
    if (j > nchar(s)) print(s) ##
    stopifnot(j <= nchar(s))
    name <- substr(s, 1, j -1)
...
# c("*/RandomFieldsUtils/src/AutoRandomFieldsUtils.h", "src/AutoRandomFields.h")
rfGenerateConstants <-
  function(package="RandomFields", aux.package = "RandomFieldsUtils",
    ...
    Package <- paste0(toupper(substr(package, 1, 1)), substring(package, 2))
    Aux.package <- paste0(toupper(substr(aux.package, 1, 1)),
                         substr(aux.package, 2, 99))
...
rfGenerateConstantsHeader <- function(RFpath, RCauto.file, header.source,
  ...
  while (i <= length(s)) {
    ...
    if (value[1] != "")
      ...
      if (Leerzeile) write(file = RCauto.file, append = TRUE, "")
      Leerzeile <- FALSE
   }
...
rfGenerateConstantsC <- function(RFpath, RCauto.file, c.source, package="none") {
  ...
  while (i <= length(s)) {
     if (!is.null(k <- kind(s, i, "  *", c(",", "{"), " ", endofname="[",
			    stops="}"))
         ||
         !is.null(k <- kind(s, i, "*", c(",", "{"), " ", endofname="[",
			    stops="}"))
         ) {
       value <- strsplit((clean_value(k$value)), "\\{")[[1]]
  ...
  if (package == "RandomFields") {
    ...
    genuine <- genuine[genuine != "RFoptions" & genuine != "RFOPTIONS"]
...
.R.fmax <- function(...) {
  a <- list(...)
  if (length(a) == 1) {
    if (isS4(a[[1]]) && is(a[[1]], CLASS_CLIST)) a[[1]] else R.c(a[[1]])
  } else R.fmax(if (isS4(a[[1]]) && is(a[[1]], CLASS_CLIST)) a[[1]]
                else R.c(a[[1]]),
                do.call(".R.fmax", a[-1]))
}
...
.R.fmin <- function(...) {
  ...
  if (length(a) == 1) {
    if (isS4(a[[1]]) && is(a[[1]], CLASS_CLIST)) a[[1]] else R.c(a[[1]])
  } else R.fmin(if (isS4(a[[1]]) && is(a[[1]], CLASS_CLIST)) a[[1]]
                else R.c(a[[1]]),
                do.call(".R.fmin", a[-1]))
}
		  ...
		  if (exists(Rname, envir=parent.env(parent.env(.GlobalEnv)))) {
		    if (args != "...")
		      ...
		      if (FALSE)
			    ...
			    args, "), function(x) isS4(x) && is(x, CLASS_CLIST),
			    ))) .", prefix, name, 
