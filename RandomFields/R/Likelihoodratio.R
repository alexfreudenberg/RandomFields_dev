...
approx_test <- function(modellist, alpha) {
  ...
  for (m in 1:n) {
    if (is(modellist[[m]], CLASS_FITLIST)) {
      if (isS4(modellist)) {
...
approx_test_single <- function(model, method, alpha, modelinfo) {
  ...
  for (i in c("submodels", "number.of.parameters", "likelihood", "report",
    ...
    assign(i, do.call(OP, list(model, i)))
  
#  submodels <- do.call(OP, list(model, "submodels"))
#  number.of.parameters <- do.call(OP, list(model, "number.of.parameters"))
#  likelihood <- do.call(OP, list(m, "likelihood"))
#  report <- do.call(OP, list(model, "report"))
#  p.proj <- do.call(OP, list(model, "p.proj"))
#  v.proj <- do.call(OP, list(model, "v.proj"))
#  x.proj <- do.call(OP, list(model, "x.proj"))
#  true.tsdim <- do.call(OP, list(model, "true.tsdim"))
#  true.vdim <- do.call(OP, list(model, "true.vdim"))
#  AIC <- do.call(OP, list(m, "AIC"))
#  BIC <- do.call(OP, list(m, "BIC"))
#  number.of.data <- do.call(OP, list(model, "number.of.data"))
#  modelAsList <-do.call(OP, list(m, "modelAsList"))
  
  if (missing(modelinfo)) modelinfo <- do.call(OP, list(model, "modelinfo"))
  fixed <- NULL
   
  if (!is.logical(x.proj) && length(x.proj)  != true.tsdim) ## todo x.proj!=NULL streichen
    stop("space-time projection can't be evaluated yet.", CONTACT)
 ...
 if (length(p.proj) > 0) proj.txt <- paste(proj.txt, ")", sep="")
  ...
  if (length(submodels) > 0) {
    ...
    while(i <= len) {
      ...
      if (true.vdim != length(tot.v.proj[[1]])) {
        ...
        for (k in 1:length(tot.models)) {
          aux.model[[k+1]] <-
            list(RM_MATRIX, M=diag(true.vdim)[, tot.v.proj[[k]], drop=FALSE],
                 tot.models[[k]])
        }
        result.model[[result.n]] <- aux.model
       } else {
        result.model[[result.n]] <- tot.models[[1]]
        if (i!=j) stop("model mismatch.", CONTACT)
      }
      ...
      if (length(subpproj) == length(unique(subpproj))) {
        delta.df <- number.of.parameters - tot.df
        if (delta.df <= 0) stop("negative df.", CONTACT)
        p <-  pchisq(2 * (likelihood - tot.loglik), df=delta.df, lower.tail = FALSE)
      } else {
        delta.df <- p <- NA
      }
      result <-
        rbind(result,
              data.frame(model1.name = tot.proj.txt,
                         ...
                         model2.loglik=likelihood,
                         model2.df = number.of.parameters,
                         model2.AIC = AIC,
                         model2.BIC = BIC,
    ...
    return(list(df=number.of.parameters, loglik=likelihood, report=report,
                p.proj = p.proj, v.proj=v.proj, AIC=AIC, BIC=BIC,
                fixed = fixed,
                fitted.model=c(result.model, list(modelAsList)),
                proj.txt=proj.txt, result = result))
    
  } else {
    return(list(df=number.of.parameters, loglik=likelihood, report=report,
                p.proj = p.proj, v.proj=v.proj, AIC=AIC, BIC=BIC,
                fixed = fixed, fitted.model=list(modelAsList),
                proj.txt = proj.txt,               
                result = data.frame(
                  model1.name = "",
                  model1.loglik = NA,
...
RFratiotest <- function(nullmodel, alternative, ## no params as output of RFfit
  ...
  if (is(nullmodel, CLASS_FITLIST)) {
    if (!missing(alternative) && !(is(alternative, CLASS_FITLIST)))
      stop("alternative model not an output of a fitting method.")
    if (!RFopt$fit$ratiotest_approx)
      stop("for results from a fitting method the parameter 'ratiotest_approx' must be'TRUE'")
  ...
  fit <- numeric(2)
  if (length(Z$coords) > 1)
    stop("multisets of data cannot be considered yet")

  Coords <- Z$coords[[1]]
  dist.given <- Coords$dist.given
  newx <- if (!dist.given) Coords$x # lapply(Z$coords, function(x) x$x)
  newT <- if (!dist.given) Coords$T # lapply(Z$coords, function(x) x$T)
