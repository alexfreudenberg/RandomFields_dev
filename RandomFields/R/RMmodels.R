...
RMplus <- new(CLASS_RM, 
	...
	isotropy = c('framework or submodel dependent'),
	domain = c('framework or submodel dependent'),
	operator = TRUE,
...
RMS <- new(CLASS_RM, 
	...
	isotropy = c('framework or submodel dependent', 'framework or submodel dependent'),
	domain = c('framework or submodel dependent'),
	operator = TRUE,
	monotone = 'submodel dependent monotonicity',
	finiterange = NA,
...
RMstein <- function(nu, zeta, var, scale, Aniso, proj) {
  ...
  if (hasArg('nu') && !is.null(subst <- substitute(nu))) 
	par.model[['nu']] <- CheckArg(nu, subst, TRUE)
  if (hasArg('zeta') && !is.null(subst <- substitute(zeta))) 
	par.model[['zeta']] <- CheckArg(zeta, subst, TRUE)
    if (hasArg('var') && !is.null(subst <- substitute(var))) 
	par.general[['var']] <- CheckArg(var, subst, TRUE)
    if (hasArg('scale') && !is.null(subst <- substitute(scale))) 
	par.general[['scale']] <- CheckArg(scale, subst, TRUE)
...
iRFpseudomadogram <- new(CLASS_RM, 
	.Data = iRFpseudomadogram,
	type = c('interface'),
	isotropy = c('parameter dependent'),
	domain = c('single variable'),
	operator = TRUE,
...
iR.c <- new(CLASS_RM, 
	.Data = iR.c,
	type = c('shape function', 'trend'),
	isotropy = c('framework or submodel dependent', 'submodel dependent'),
	domain = c('framework or submodel dependent'),
	operator = FALSE,
	monotone = 'not monotone',
	finiterange = FALSE,
